...
public class Clipper {
    ...
    public Clipper(double near_plane, double far_plane, double x_low, double y_low,
    ...
    private Vertex3D determine_new_point_under_perspective(Vertex3D p1, Vertex3D p2) {
        ...
        if(!near_plane_clip && !far_plane_clip) {
            p1 = multiply_perspective_matrix(p1);
        }
        return p1;
    }
    ...
    private Vertex3D multiply_perspective_matrix(Vertex3D current) {
        double[][] perspective_matrix = SimpInterpreter.toPerspective;
        ...
        Point3DH temp_point = new Point3DH(current_point[0], current_point[1], current_point[2], current_point[3]).euclidean();
        temp_point.setZ(current.getZ());
        return new Vertex3D(temp_point, color);
    }
    ...
    public ArrayList<Vertex3D> getVertex(){
        double[][] view_to_screen = SimpInterpreter.toView;
        ...
        for (Vertex3D current : points_list) {
            ...
            for (int i = 0; i < 4; i++) {
                double lineTotal = 0;
                for (int j = 0; j < 4; j++) {
                    lineTotal += view_to_screen[i][j] * current_point[j];
                }
                temp[i] = lineTotal;
            }
            current_point = temp;
